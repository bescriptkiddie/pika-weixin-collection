...
def main() -> None:
    ...
    parser.add_argument(
        "--no-example-fallback",
        action="store_true",
        help="Do not sync from data/external_sources.example.json when data/external_sources.json is missing.",
    )
    parser.add_argument(
        "--source-id",
        action="append",
        default=[],
        help="Only sync one source id. Repeat or pass comma-separated ids for multiple sources.",
    )
    args = parser.parse_args()
    source_ids = [
        source_id.strip()
        for raw in args.source_id

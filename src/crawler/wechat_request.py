...
class WechatRequest:
    ...
    def _get_json(self, url: str, params: dict[str, Any], timeout: int) -> dict[str, Any]:
        response = self.session.get(url=url, params=params, headers=self.headers, timeout=timeout)
    ...
    def name2fakeid(self, name):
        ...
        response = self._get_json(url=url, params=params, timeout=15)
    ...
    def fakeid2message_update(self, fakeid, message_exist=[]):
        ...
        msgid_exist = {m['id'] for m in message_exist}
        ...
        page = 0
        while True:
            params['begin'] = page * params['count']
            response = self._get_json(url=url, params=params, timeout=30)
            ...
            for published in publish_list:
                message = json.loads(published['publish_info'])
                for appmsg in message['appmsgex']:
                    unique_id = f"{message['msgid']}-{appmsg['aid']}-{appmsg['create_time']}"

                    if time_delta(time_now(), jstime2realtime(appmsg['create_time'])).days > 60:
                        reached_time_limit = True
                        continue

                    if unique_id in skipped_by_user or unique_id in msgid_exist:
                        continue

                    if appmsg['is_deleted']:
                        if unique_id not in is_deleted_set:
                            data_manager.issues_message['is_delete'].append(unique_id)
                            is_deleted_set.add(unique_id)
                        continue

                    if appmsg['item_show_type'] in [5, 8, 10]:
                        continue
                    ...
                    crawl_message_info.append(asdict(Message_Info(
                        ...
                        link=appmsg['link'],
                        cover=appmsg['cover'],
                        create_time=jstime2realtime(appmsg['create_time']),
                        is_deleted=appmsg['is_deleted'],
                        item_show_type=appmsg['item_show_type'],
                    )))

            if reached_time_limit or not page_has_new:
                break
    ...
    def session_is_overdue(self, response):
        ...
        auto_login=True（默认）：凭证过期时自动调用 login() 重新登录（交互式场景）
        auto_login=False：凭证过期时直接抛异常，由调用方处理（服务器爬取场景）
        ...
        if err_msg in ['invalid session', 'invalid csrf token']:
            if not self.auto_login:
                raise Exception(f'凭证已失效（{err_msg}），请在配置页扫码重新登录')
